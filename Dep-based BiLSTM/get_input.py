...
# -*- coding: utf-8 -*-
"""
Created on Fri May 20 09:50:27 2022
...
def generate_text_entities_and_label(data):
    ...
    entity1 = '{0}/{1}'.format(name_i.lower(),i)
    entity2 = '{0}/{1}'.format(name_j.lower(),j)
...
def find_root_and_id(sent_dict):
    ...
    for word in sent_dict:
        if word['head'] == 0:
            root=word['text']
            root_id=word['id']-1
    root_entity = '{0}/{1}'.format(root.lower(),int(root_id))
    return root_entity

#root,root_id = find_root_and_id(sent_dict)
#print(root,root_id)
#v2
#root_entity = find_root_and_id(sent_dict)
...
def find_shortest_subpath(doc,entity1,entity2,root_entity):
    edges = []
    for token in doc.sentences[0].dependencies:
        if token[0].text.lower() != 'root':
            #edges.append((token[0].text.lower(), token[2].text))
            ...
            edges.append((starting,ending))
    graph = nx.Graph(edges)   
    SDP1 = nx.shortest_path(graph, source=entity1, target=root_entity)
    SDP2 = nx.shortest_path(graph, source=entity2, target=root_entity)
    return SDP1,SDP2

#v2
#SDP1,SDP2 = find_shortest_path(doc,entity1,entity2,root_entity)
#SDP = find_shortest_path(doc,entity1,entity2)
#print(SDP)
def restructure_SDP(SDP):
    new = []
    for elem in SDP:
        text, index = elem.split('/')
        index = int(index)
        new.append(text)
    return new
...
#v2
#dep_relation1,pos_tags1 = find_dep_relations_and_pos(SDP1, sent_dict)
#dep_relation2,pos_tags2 = find_dep_relations_and_pos(SDP2, sent_dict)
...
for data in training_set:
    ...
    root_entity = find_root_and_id(sent_dict)
    #print(root_entity)
    root,root_id = root_entity.split('/')
    #print(root_id)
    SDP = find_shortest_path(doc, entity1, entity2)
    #print(SDP)
    root_index = find_root_index_in_SDP(SDP,root_id)
    #print(root_index)
    # root in SDP between two entities
    if root_index != -1:
        #print('root in SDP')
        dep_relations,pos_tags = find_dep_relations_and_pos(SDP,sent_dict)
        subpath1 ,subpath2 = generate_two_subpaths(SDP,root_index,dep_relations,pos_tags) 
      ...
      #  print('\n')
      #  print(subpath2)
        x_train.append([subpath1,subpath2])
    else:
        #print('root not in SDP')
        subpath1=[]
        subpath2 = []
        SDP1,SDP2 = find_shortest_subpath(doc,entity1,entity2,root_entity)
        #print(SDP1,'\n',SDP2)
        #visualization(sent_dict)
        dep_relation1,pos_tags1 = find_dep_relations_and_pos(SDP1, sent_dict)
        dep_relation2,pos_tags2 = find_dep_relations_and_pos(SDP2, sent_dict)    
        SDP1,SDP2 = restructure_SDP(SDP1),restructure_SDP(SDP2)
        subpath1 += [SDP1,dep_relation1,pos_tags1]
        subpath2 += [SDP2,dep_relation2,pos_tags2]
        x_train.append([subpath1,subpath2])
    end = time.time()
    print(end-start)
print('done')
...
for elem in x_train:
    subpath1 = elem[0]
    subpath2 = elem[1]
    training_word.append(subpath1[0])
    training_word.append(subpath2[0])
    training_dep.append(subpath1[1])
    training_dep.append(subpath2[1])
    training_pos.append(subpath1[2])
    training_pos.append(subpath2[2])

print('training_word',training_word)
print('training_dep',training_dep)
print('training_pos',training_pos)
...
for elem in test_set:
    ...
    if root_index != -1:
      ...
      #  print('\n')
      #  print(subpath2)
        x_test.append([subpath1,subpath2])
    else:
        #print('root not in SDP')
        subpath1=[]
        subpath2 = []
        ...
        x_test.append([subpath1,subpath2])
      #  print(SDP1,'\n',SDP2)
      #  print(dep_relation1,pos_tags1)
      #  print(dep_relation2,pos_tags2)
   # print(x_train)
print('x_test',x_test)
testing_word = []
